...
def summarize_memory():
    ...
    # 【Fix 1】添加读取锁，防止读取时文件正在被写入
    with get_chat_lock(chat_id):
        history = load_chat_data(chat_id)

    # 辅助函数：安全转换整数，防止 JSON 中存的是字符串 "1" 导致 1 == "1" 失败
    def is_type_match(m, target_type=1):
        try:
            val = m.get('memory_type', 1)
            return int(val) == target_type
    ...
    to_summary_list = []
    for m in history:
        # 1. 必须是记忆开启状态 (兼容 is_memory 缺失的情况默认为 True)
        # 注意：这里显式判断 is False，防止 0 或 None 的意外干扰
        if m.get('is_memory', True) is False:
            continue
            
        # 2. 必须是 (临时记忆 Type=1) 或 (角色是 summary)
        if is_type_match(m, 1) or m.get('role') == 'summary':
            to_summary_list.append(m)

    text_to_summarize = "\n".join([f"{m.get('role', 'unknown')}: {m.get('content', '')}" for m in to_summary_list])
    
    # 调试打印：如果你再次遇到问题，看控制台输出
    print(f"[Debug Summarize] Chat: {chat_id}, Total: {len(history)}, Filtered: {len(to_summary_list)}")

    if not text_to_summarize.strip():
        # 细化报错信息，方便前端判断
        return jsonify({
            "error": "当前没有可浓缩的临时记忆", 
            "details": f"Total msgs: {len(history)}, Valid targets: {len(to_summary_list)}"
        }), 400
    ...
    try:
        ...
        if not model_name:
            if api_type == "google":
                # 修正：使用了更稳定的模型名称，避免 preview 版下线导致报错
                model_name = 'gemini-2.0-flash' 
            else:
                model_name = PROVIDER_SPECS[ProviderType.OPENAI].default_model

        provider = get_provider(api_type, int(api_key_index) if api_key_index is not None and str(api_key_index).isdigit() else None)
...
def ai_discuss():
    ...
    #    然后直接拼接 Seed Prompt。
    # =========================
    def build_iteration_prompt(round_no: int, total_rounds: int, current_order: int, total_participants: int,
                               display_name: str, real_model: str) -> str:
        ...
        return (
            f"[System Meta: {role_info}{role_instruction}]\n"
            f"[Context Info: Round {round_no} of {total_rounds} | My Order: {current_order} of {total_participants} in this round]\n\n"
            f"{seed_prompt.strip()}"
        )
